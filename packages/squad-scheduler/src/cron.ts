...
function parseName(s: string, map: Record<string, number>): number {
  const v = map[s.toUpperCase()];
  if (v !== undefined) return v;
  const n = parseInt(s, 10);
  if (isNaN(n)) throw new Error(`Invalid cron token: ${s}`);
  return n;
}

function expandField(field: string, min: number, max: number, nameMap?: Record<string, number>): Set<number> {
  const result = new Set<number>();
  const parse = (s: string) => nameMap ? parseName(s, nameMap) : parseInt(s, 10);
...
export function nextDue(cron: ParsedCron, after: Date = new Date()): Date {
...
export function describeCron(expr: string): string {
  try {
    ...
    if (c.minutes.size === 60) return hrArr.length === 24 ? 'every minute' : `every hour`;
    if (c.minutes.size === 1 && c.hours.size === 24) return `every hour at :${String(minArr[0]).padStart(2,'0')}`;
