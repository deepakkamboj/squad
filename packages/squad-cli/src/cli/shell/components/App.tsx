  ...
  return (
    <Box flexDirection="column">
      ...
      <Box marginTop={1} borderStyle={noColor ? undefined : 'round'} borderColor={noColor ? undefined : 'cyan'} paddingX={1}>
        <InputPrompt onSubmit={handleSubmit} disabled={processing} agents={welcome?.agents ? welcome.agents.map(a => ({ name: a.name, description: a.role })) : agents.map(a => ({ name: a.name, description: a.role }))} messageCount={messages.length} />
      </Box>
